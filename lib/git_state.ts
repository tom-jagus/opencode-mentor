...
export type GitNotRepositoryState = {
  version: 1;
  available: true;
  repository: false;
  directory: string;
...
export type GitInspectionFailure = {
  ...
  repository: null;
  directory: string;
  reason: "git-inspection-failed" | "git-unavailable";
  error: string;
};
...
export function parseGitStatus(output: string): ParsedStatus {
  ...
  const untracked = changes
    .filter(
      (change) => change.index_status === "?" && change.worktree_status === "?",
    )
    .map((change) => change.path);

  const conflicts = changes
    .filter((change) =>
      conflictCodes.has(`${change.index_status}${change.worktree_status}`),
    )
    .map(displayPath);
...
export async function inspectGitState(directory: string): Promise<GitState> {
  try {
    ...
    if (rootResult.exitCode !== 0) {
      ...
      return {
        ...
        error: error || `git rev-parse exited with code ${rootResult.exitCode}`,
      };
    }
    ...
    if (hasHead && branch) {
      ...
      if (upstreamResult.exitCode === 0) {
        ...
        if (countResult.exitCode === 0) {
          const [aheadValue, behindValue] = countResult.stdout
            .trim()
            .split(/\s+/);
    ...
    if (hasHead) {
      ...
      if (logResult.exitCode === 0) {
        const fields = logResult.stdout.replace(/\n$/, "").split("\0");
