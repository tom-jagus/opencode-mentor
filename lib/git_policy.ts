...
export type EffectiveGitPolicyResolution = {
  project_root: string;
  sources: {
    global: {
      path: string;
...
function stringValue(value: unknown, path: string, source: string): string {
...
function tableValue(value: unknown, path: string, source: string): PolicyTable {
  if (!isTable(value)) {
    throw new PolicyError(
      "INVALID_POLICY_VALUE",
...
function normalizePolicy(
  ...
  const schemaVersion = requiredValue(root, "schema_version", "policy", source);
  ...
  if (baseBranch !== undefined) {
    const normalized = stringValue(baseBranch, "policy.base_branch", source);
...
function normalizeBranch(
  ...
  const value = optionalValue(root, "branch", partial, "policy", source);
...
function normalizeMerge(
  ...
  const value = optionalValue(root, "merge", partial, "policy", source);
...
function normalizeBranchUpdate(
  ...
  const value = optionalValue(root, "branch_update", partial, "policy", source);
  ...
  assertKnownKeys(
    table,
    new Set(["strategy", "require_before_finalization", "force_push"]),
...
function normalizeValidation(
  ...
  const value = optionalValue(root, "validation", partial, "policy", source);
  ...
  assertKnownKeys(table, new Set(["profile"]), "policy.validation", source);
...
function normalizePullRequest(
  ...
  const value = optionalValue(root, "pull_request", partial, "policy", source);
  ...
  for (const key of ["draft", "generated_body"] as const) {
    ...
    if (setting !== undefined) {
      output[key] = booleanValue(setting, `policy.pull_request.${key}`, source);
    }
  }
...
function normalizeRelease(
  ...
  const value = optionalValue(root, "release", partial, "policy", source);
  ...
  assertKnownKeys(
    table,
    new Set(["enabled", "versioning", "tag_prefix", "notes"]),
    "policy.release",
    source,
  );
  ...
  if (enabled !== undefined) {
    output.enabled = booleanValue(enabled, "policy.release.enabled", source);
  }
...
export function policyFailure(error: unknown): {
  version: 1;
  ok: false;
  error: {
    code: PolicyErrorCode;
  ...
  if (error instanceof PolicyError) {
    return {
      ...
      error: {
        ...
        ...(error.policy_path ? { path: error.policy_path } : {}),
      },
    };
  }
...
export async function resolveEffectiveGitPolicy(
  directory: string,
  configurationRoot: string,
): Promise<EffectiveGitPolicyResolution> {
  let projectRoot: string;
  let resolvedConfigurationRoot: string;

  try {
    [projectRoot, resolvedConfigurationRoot] = await Promise.all([
      realpath(directory),
      realpath(configurationRoot),
    ]);
  } catch (error) {
    throw new PolicyError(
      "PROJECT_RESOLUTION_FAILED",
  ...
  const globalPath = resolve(
    resolvedConfigurationRoot,
    "policies",
    "git-defaults.toml",
  );
  const projectPath = resolve(projectRoot, ".opencode", "git-policy.toml");
  ...
  const [globalText, projectText] = await Promise.all([
    ...
    readPolicyFile(projectPath, projectRoot, false, "project policy"),
  ]);
